...
def load_titanic_data():
    ...
    return train_data, test_data, 'survived'
...
def load_credit_card_data():
    ...
    return train_data, test_data, 'Class'
...
def individual_model_hpo(train_data, test_data, experiment_name, model_types, num_trials, timeout, target_column):
    """개별 모델 최적화 (통합 DB 사용)"""
    ...
    for model_type in model_types:
        ...
        def objective(trial):
            ...
            # AutoGluon Predictor 설정 - target_column 동적 사용
            predictor = TabularPredictor(
                label=target_column,  # 동적으로 타겟 컬럼 설정
                problem_type='binary',
                eval_metric='f1',
...
def final_ensemble_with_autogluon(train_data, test_data, best_params, target_column):
    ...
    # 테스트 데이터로 평가 - target_column 동적 사용
    ...
    test_f1 = f1_score(test_data[target_column], test_predictions)
    test_acc = accuracy_score(test_data[target_column], test_predictions)
    test_prec = precision_score(test_data[target_column], test_predictions)
    test_rec = recall_score(test_data[target_column], test_predictions)
...
def run_single_stage_hpo_unified_db(experiment_name, model_types, num_trials, timeout, dataset_name):
    """1단계 HPO + AutoGluon 자동 앙상블 실행 (통합 DB)"""
    print(f"🚀 1단계 HPO + AutoGluon 자동 앙상블 실험 시작: {experiment_name}")
    
    # 데이터 로드 (데이터셋 선택)
    train_data, test_data, target_column = load_data(dataset_name)
    
    # 1단계: 개별 모델 최적화 (통합 DB 사용) - model_types 전달
    best_params = individual_model_hpo(train_data, test_data, experiment_name, model_types, num_trials, timeout, target_column)
    ...
    final_predictor, test_metrics = final_ensemble_with_autogluon(train_data, test_data, best_params, target_column)
...
def main():
    # 명령행 인자 파싱
    parser = argparse.ArgumentParser(description='Optuna HPO with Multi-Dataset Support')
    parser.add_argument('experiment_name', type=str, help='실험 이름')
    parser.add_argument('dataset_name', type=str, choices=['titanic', 'creditcard'], help='데이터셋 이름')
    ...
    if args.models:
        ...
        model_types = [
            ...
            'CUSTOM_NN_TORCH',  'ADAFOCAL_DL', 'RF' # 'ADVANCED_FOCAL_DL',
        ]
    ...
    # 데이터셋별 설명
    if dataset_name.lower() == 'titanic':
        print(f"🎯 데이터셋: Titanic (이진 분류, 타겟: survived)")
    elif dataset_name.lower() == 'creditcard':
        print(f"🎯 데이터셋: Credit Card Fraud Detection (극도로 불균형, 타겟: Class)")
    
    print(f"⏱️  각 모델당 {num_trials} trials, 총 {num_trials * len(model_types)} trials")
    
    # HPO 실행 - model_types 전달
    run_single_stage_hpo_unified_db(experiment_name, model_types, num_trials, timeout, dataset_name)
