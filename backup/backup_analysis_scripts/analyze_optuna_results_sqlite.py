...
def get_all_trials_from_sqlite():
    """SQLite DB에서 각 모델별 모든 trial 데이터 수집"""
    model_types = ['DCNV2', 'CUSTOM_FOCAL_DL', 'RF']
    all_trials_data = {}
    
    for model_type in model_types:
        print(f"\n🔍 {model_type} SQLite DB에서 trial 데이터 수집 중...")
        
        try:
            # SQLite DB에서 study 로드
            ...
            print(f"   📊 총 {len(study.trials)}개 trial 중 {len(successful_trials)}개 완료")
            ...
            if not trials_data:
                print(f"   ❌ {model_type}: 완료된 trial이 없습니다")
            else:
                print(f"   ✅ {model_type}: {len(trials_data)}개 trial 데이터 수집 완료")
                print(f"   📈 성능 범위: {min(t['F1_Score'] for t in trials_data):.4f} ~ {max(t['F1_Score'] for t in trials_data):.4f}")
                print(f"   🏆 최고 성능: {study.best_value:.4f}")
                print(f"   ⏱️ 평균 소요시간: {np.mean([t['Duration'] for t in trials_data]):.1f}초")
...
def create_detailed_sqlite_report():
    """SQLite 기반 상세 리포트 생성"""
    print("=== Optuna HPO SQLite 상세 결과 분석 ===")
    
    # 1. SQLite에서 모든 trial 데이터 수집
    all_trials_data = get_all_trials_from_sqlite()
    ...
    filename = f"optuna_sqlite_results_{timestamp}.xlsx"
    ...
    print(f"\n✅ SQLite 기반 상세 결과가 '{filename}'에 저장되었습니다!")
...
if __name__ == "__main__":
    individual_df, ensemble_df, all_trials_data = create_detailed_sqlite_report()
    
    print("\n🎉 SQLite 기반 분석 완료!")
    print("📋 엑셀 파일에 다음 시트들이 포함됩니다:")
    print("  - Best_Results: 각 모델의 최고 성능 결과")
    print("  - DCNV2_All_Trials: DCNV2의 모든 실제 trial")
    print("  - CUSTOM_FOCAL_DL_All_Trials: Focal Loss의 모든 실제 trial")
    print("  - RF_All_Trials: RandomForest의 모든 실제 trial")
