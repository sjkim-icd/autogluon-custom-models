...
def create_complete_dashboard(studies):
    """완전한 대시보드 생성 (원래 차트 + 새로운 차트 + 필터)"""
    print("=== 완전한 대시보드 생성 ===")
    ...
    <head>
        ...
        <title>🎯 Optuna HPO 완전한 대시보드</title>
    ...
    <body>
        <div class="container">
            <div class="header">
                <h1>🎯 Optuna HPO 완전한 대시보드</h1>
                <p>원래 차트 + 고급 시각화 + 필터 기능</p>
    ...
    html_content += """
            ...
            <div class="footer">
                <p>🎯 Optuna HPO 완전한 대시보드 | 생성 시간: """ + datetime.now().strftime('%Y-%m-%d %H:%M:%S') + """</p>
            </div>
        </div>
        ...
        <script>
            ...
            function applyFilter(modelType) {
                ...
                // 6개 차트 모두 업데이트
                updateOptimizationHistory(modelType, selectedParam, selectedValue);
                updateParameterImportance(modelType, selectedParam, selectedValue);
                updateParameterCorrelation(modelType, selectedParam, selectedValue);
            ...
            // 차트 업데이트 함수들 (필터 적용 시)
            function updateOptimizationHistory(modelType, selectedParam, selectedValue) {
                console.log('최적화 과정 업데이트:', modelType, selectedParam, selectedValue);
            }
            
            function updateParameterImportance(modelType, selectedParam, selectedValue) {
                console.log('파라미터 중요도 업데이트:', modelType, selectedParam, selectedValue);
            }
            
            function updateParameterCorrelation(modelType, selectedParam, selectedValue) {
                console.log('파라미터 상관관계 업데이트:', modelType, selectedParam, selectedValue);
            }
    ...
    filename = f"optuna_complete_dashboard_{timestamp}.html"
    
    with open(filename, 'w', encoding='utf-8') as f:
        f.write(html_content)
    
    print(f"✅ 완전한 대시보드가 '{filename}'에 저장되었습니다!")
    return filename
...
if __name__ == "__main__":
    ...
    if not studies:
        ...
        # 완전한 대시보드 생성
        dashboard_file = create_complete_dashboard(studies)
