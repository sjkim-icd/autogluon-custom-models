...
def create_excel_report_unified_db(studies, importance_results, history_results):
    """고급 분석 리포트 엑셀 생성 (통합 DB 버전)"""
    ...
    filename = f"optuna_advanced_report_unified_db_{timestamp}.xlsx"
    wb.save(filename)
...
def create_summary_sheet(wb, studies, history_results):
    ...
    # 제목
    ws['A1'] = "🎯 Optuna HPO 고급 분석 리포트 (통합 DB)"
    ...
    # DB 정보
    ws['A3'] = "DB 파일: optuna_studies/all_studies.db"
    ws['A3'].font = Font(size=10, italic=True, color="0066CC")
...
def create_unified_db_dashboard_with_custom_recommendations(studies):
    """통합 DB용 대시보드 생성 (모든 기존 기능 + 사용자 지정 권장사항)"""
    print("=== 통합 DB 대시보드 생성 (모든 기능 + 사용자 지정 권장사항) ===")
            ...
            if len(numeric_params) >= 2:
                ...
                for i, param1 in enumerate(param_names):
                    for j, param2 in enumerate(param_names):
                        if i < j and len(numeric_params[param1]) == len(numeric_params[param2]):  # 중복 제거 (i < j)
    ...
    # HTML 시작 (기존 create_unified_db_dashboard.py와 동일한 구조)
    html_content = """
    <!DOCTYPE html>
    <html lang="ko">
    <head>
        ...
        <title>🎯 Optuna HPO 통합 DB 대시보드 (완전판)</title>
    ...
    <body>
        <div class="container">
            <div class="header">
                <h1>🎯 Optuna HPO 통합 DB 대시보드 (완전판)</h1>
                <p>all_studies.db 기반 | 모든 기능 + 사용자 지정 권장사항 + 고급 엑셀 보고서</p>
    ...
    for model_type, study in studies.items():
        ...
        html_content += f"""
                    <h4>🔗 파라미터 상관관계</h4>
                    <div class="chart-container">
                        <div id="correlation_{model_type}" style="height: 400px;"></div>
                    </div>
        """
        
        # 4. Parallel Coordinate (개별 필터 포함)
        html_content += f"""
    ...
    # JavaScript 코드 (기존과 동일)
    html_content += """
            </div>
            
            <div class="footer">
                <p>🎯 Optuna HPO 통합 DB 대시보드 (완전판) | 생성 시간: """ + datetime.now().strftime('%Y-%m-%d %H:%M:%S') + """</p>
        ...
        <script>
                ...
                const trace = {
                    ...
                    marker: { 
                        color: sortedCorr.map(item => {
                            if (item[1] > 0.7) return '#d32f2f';      // 강한 양의 상관: 빨간색
                            else if (item[1] > 0.3) return '#f57c00'; // 중간 양의 상관: 주황색
                            else if (item[1] > 0) return '#388e3c';   // 약한 양의 상관: 녹색
                            else if (item[1] > -0.3) return '#1976d2'; // 약한 음의 상관: 파란색
                            else if (item[1] > -0.7) return '#7b1fa2'; // 중간 음의 상관: 보라색
            ...
            window.addEventListener('load', function() {
                console.log('통합 DB 대시보드 (완전판) 로드 완료');
    ...
    # HTML 파일 저장
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"optuna_complete_unified_dashboard_{timestamp}.html"
    
    with open(filename, 'w', encoding='utf-8') as f:
        f.write(html_content)
    
    print(f"✅ 통합 DB 대시보드 (완전판)가 '{filename}'에 저장되었습니다!")
    return filename
...
if __name__ == "__main__":
    ...
    if not studies:
        ...
        # 1. HTML 대시보드 생성 (모든 기능 + 사용자 지정 권장사항)
        html_file = create_unified_db_dashboard_with_custom_recommendations(studies)
        
        # 2. 엑셀 보고서 생성 (기존 고급 서식과 동일)
        excel_file = create_excel_report_unified_db(studies, importance_results, history_results)
        
        print("\n🎉 통합 DB 기반 완전한 분석 완료!")
        print("📋 HTML 대시보드 포함 기능:")
        print("  ✅ 통합 DB(all_studies.db) 기반 데이터 로드")
        print("  ✅ 기존 차트들: 최적화 과정, 파라미터 중요도, 파라미터 상관관계") 
        print("  ✅ 새로운 차트들: Parallel Coordinate, Contour Plot, Slice Plot")
        print("  ✅ 각 차트별 독립적인 필터 패널")
        print("  ✅ 사용자 지정 형식의 권장사항 (기준 설명 + 모델별 맞춤 권장)")
        print("  ✅ 📂 통합 DB 정보 표시")
        print("\n📊 엑셀 보고서 포함 기능:")
        print("  ✅ 📊 실험 요약: 전체 통계 + 모델별 성능 비교")
        print("  ✅ 🎯 파라미터 중요도: 모델별 파라미터 영향도 분석")
        print("  ✅ 📈 최적화 과정: 수렴성 분석 + 조건부 서식")
        print("  ✅ 📋 모델별 상세: 모든 trial 데이터 + 최고 성능 강조")
        print("  ✅ 💡 권장사항: 분석 기준 설명 + 모델별 맞춤 권장")
        print("  ✅ 조건부 서식: 성능별 색상 구분 + 중요도별 하이라이트")
        print(f"\n📂 생성된 파일:")
        print(f"  HTML: {html_file}")
        print(f"  Excel: {excel_file}")
        print("🌐 웹 브라우저에서 HTML을 열어서 모든 상호작용 기능을 확인하세요!")
        print("📊 엑셀 파일에서 조건부 서식과 상세 데이터를 확인하세요!") 