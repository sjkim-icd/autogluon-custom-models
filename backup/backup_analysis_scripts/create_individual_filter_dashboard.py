...
def create_individual_filter_dashboard(studies):
    """각 차트별 개별 필터가 있는 대시보드 생성"""
    print("=== 개별 필터 대시보드 생성 ===")
    ...
    <head>
        ...
        <title>🎯 Optuna HPO 개별 필터 대시보드</title>
    ...
    <body>
        <div class="container">
            <div class="header">
                <h1>🎯 Optuna HPO 개별 필터 대시보드</h1>
                <p>각 차트별 독립적인 필터 + 권장사항</p>
    ...
    html_content += """
            ...
            <div class="footer">
                <p>🎯 Optuna HPO 개별 필터 대시보드 | 생성 시간: """ + datetime.now().strftime('%Y-%m-%d %H:%M:%S') + """</p>
            </div>
        </div>
        ...
        <script>
            ...
            function updateSlicePlot(modelType) {
                ...
                if (filteredData.length > 0) {
                    ...
                    const layout = {
                        title: modelType + ' Slice Plot (' + sliceParam + ') - 필터링됨',
    ...
    filename = f"optuna_individual_filter_dashboard_{timestamp}.html"
    
    with open(filename, 'w', encoding='utf-8') as f:
        f.write(html_content)
    
    print(f"✅ 개별 필터 대시보드가 '{filename}'에 저장되었습니다!")
    return filename
...
if __name__ == "__main__":
    ...
    if not studies:
        ...
        # 개별 필터 대시보드 생성
        dashboard_file = create_individual_filter_dashboard(studies)
