...
def run_five_models_hpo_experiment():
    """5개 모델 HPO 실험"""
    print("=== Titanic 데이터셋 5개 모델 HPO 실험 시작 ===")
    ...
    predictor = TabularPredictor(
        ...
        problem_type='binary',
        eval_metric='f1',
        path="models/five_models_titanic_hpo",
        verbosity=4
    )
    
    # 모델별 하이퍼파라미터 검색 공간 설정
    hyperparameters = {
        'DCNV2': {
            'learning_rate': space.Real(0.0001, 0.01, log=True),
            'weight_decay': space.Real(1e-6, 0.001, log=True),
