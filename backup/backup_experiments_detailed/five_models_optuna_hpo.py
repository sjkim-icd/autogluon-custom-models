...
def run_optuna_hpo():
    ...
    # 모델 타입별 Optuna 실험
    model_types = ['DCNV2', 'CUSTOM_FOCAL_DL', 'RandomForest']
    ...
    for model_type in model_types:
        ...
        try:
            ...
            plt.savefig(f'optuna_{model_type}_history.png')
            plt.close()
            
            optuna.visualization.plot_param_importances(study)
            plt.title(f'{model_type} Parameter Importance')
            plt.savefig(f'optuna_{model_type}_importance.png')
            plt.close()
            
            print(f"📈 시각화 저장 완료: optuna_{model_type}_*.png")
        except ImportError:
