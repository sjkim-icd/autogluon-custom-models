...
def create_folder_structure(dataset_name, output_dir="EDA", selected_packages=None):
    """
    ...
    Args:
        ...
        selected_packages (list): 선택된 패키지 리스트
    ...
    for package_name, package_path in all_packages.items():
        if selected_packages is None or package_name in selected_packages:
...
def run_autoviz(df, packages, dataset_name, target_var, max_rows=1000, max_cols=20):
    ...
    try:
        ...
        try:
            ...
            max_box_vars = min(6, len(numeric_cols_for_box))  # 2x3=6개만 가능
        ...
        if target_var and target_var in df.columns:
            try:
                ...
                # 타겟변수별 분포를 보여주는 Violin Plot
                numeric_cols_for_violin = [col for col in numeric_cols if col != target_var][:3]
                
                # 데이터 불균형 확인
                target_counts = df[target_var].value_counts()
                print(f"📊 타겟변수 분포: {dict(target_counts)}")
                
                # 불균형이 심하면 샘플링하여 균형 맞추기
                if len(target_counts) > 1 and min(target_counts) < 50:
                    print("⚖️ 데이터 불균형 감지 - 균형 샘플링 적용")
                    
                    # 소수 클래스의 샘플 수를 늘리기
                    minority_class = target_counts.idxmin()
                    minority_samples = df[df[target_var] == minority_class]
                    
                    # 소수 클래스가 너무 적으면 복원 샘플링
                    if len(minority_samples) < 50:
                        minority_samples = minority_samples.sample(n=min(200, len(minority_samples) * 20), 
                                                                 replace=True, random_state=42)
                    
                    # 다수 클래스에서 동일한 수만큼 샘플링
                    majority_class = target_counts.idxmax()
                    majority_samples = df[df[target_var] == majority_class].sample(n=len(minority_samples), 
                                                                                  random_state=42)
                    
                    balanced_df = pd.concat([majority_samples, minority_samples], ignore_index=True)
                    print(f"⚖️ 균형 샘플링 완료: {len(balanced_df)}개 (Class 0: {len(majority_samples)}, Class 1: {len(minority_samples)})")
                else:
                    balanced_df = df
                
                for i, num_var in enumerate(numeric_cols_for_violin, 1):
                    plt.subplot(1, 3, i)
                    try:
                        # 수치형 변수를 구간으로 나누어서 분석
                        df_for_violin = balanced_df.copy()
                        df_for_violin[f'{num_var}_group'] = pd.qcut(df_for_violin[num_var], 
                                                                   q=5, labels=['Q1', 'Q2', 'Q3', 'Q4', 'Q5'], 
                                                                   duplicates='drop')
                        
                        # 타겟변수별 구간별 분포
                        sns.violinplot(data=df_for_violin, x=f'{num_var}_group', y=target_var, 
                                      palette=['blue', 'red'], split=True)
                        plt.title(f'Autoviz Violin Plot: {num_var} 구간별 {target_var} 분포')
                        plt.xlabel(f'{num_var} 구간')
                        plt.ylabel(target_var)
                        plt.xticks(rotation=45)
                    except Exception as e:
                        plt.text(0.5, 0.5, f'{num_var} 구간별 {target_var}\n오류: {e}', 
...
def main():
    ...
    packages = create_folder_structure(args.dataset_name, args.output_dir, args.packages)
